...
## Step 4: Guided questions (ask, don’t assume)
...
- If answers are sufficient: summarize agreed details, update `todos/roadmap.md` with the refined/added item (keeping existing format/status markers).
...
- Next step line: `Next: /next-work {subject-slug}` (only if ready), else “Need answers on: …”.
