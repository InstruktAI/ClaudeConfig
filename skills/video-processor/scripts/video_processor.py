...
def check_whisper() -> bool:
    ...
    try:
        subprocess.run(["whisper", "--help"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        return True
    except (subprocess.CalledProcessError, FileNotFoundError):
        return False
...
def to_mp4(input_file: str, output_file: str, codec: str, preset: str, crf: int):
    ...
    try:
        ...
        stream = ffmpeg.output(
            stream, str(output_path), vcodec=codec, preset=preset, crf=crf, acodec="aac", audio_bitrate="128k"
        )
        ffmpeg.run(stream, overwrite_output=True, capture_stdout=True, capture_stderr=True)
...
def to_webm(input_file: str, output_file: str, codec: str, crf: int, bitrate: str):
    ...
    try:
        stream = ffmpeg.input(str(input_path))
        stream = ffmpeg.output(
            stream, str(output_path), vcodec=codec, crf=crf, acodec="libopus", audio_bitrate="128k", **{"b:v": bitrate}
        )
        ffmpeg.run(stream, overwrite_output=True, capture_stdout=True, capture_stderr=True)
...
@click.option("--language", default=None, help="Language code (e.g., en, es, fr). Auto-detect if not specified.")
@click.option(
    "--format",
    "output_format",
    default="txt",
...
def transcribe(
    input_file: str, output_file: str, model: str, language: Optional[str], output_format: str, verbose: bool
):
    """
