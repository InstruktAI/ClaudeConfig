...
```bash
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py extract-audio input.mp4 output.wav
```
...
```bash
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py to-mp4 input.avi output.mp4
```
...
```bash
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py to-webm input.mp4 output.webm
```
...
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe input.mp4 transcript.txt

# Transcribe audio file directly
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe audio.wav transcript.txt
```
...
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py extract-audio lecture.mp4 lecture.wav

# 2. Transcribe to SRT subtitles
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe lecture.mp4 lecture.srt --format srt --model small

# 3. Convert to web format
.venv/bin/python .claude/skills/video-processor/scripts/video_processor.py to-webm lecture.mp4 lecture.webm
```
...
1. Use the to-mp4 command with default settings:
   ```bash
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py to-mp4 old_video.avi output.mp4
   ```
...
2. Then transcribe using the base model (good balance of speed/accuracy):
   ```bash
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe lecture.mp4 transcript.txt --model base
   ```
...
1. Convert to WebM for web optimization:
   ```bash
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py to-webm presentation.mp4 presentation.webm
   ```
2. Generate SRT subtitle file:
   ```bash
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe presentation.mp4 subtitles.srt --format srt --model small
   ```
...
1. Use a larger model with language specified for best accuracy:
   ```bash
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe interview.mp4 transcript.txt --model medium --language es
   ```
2. Optionally create SRT for review:
   ```bash
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe interview.mp4 transcript.srt --format srt --model medium --language es
   ```
...
2. For each video file, run the conversion and transcription:
   ...
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py to-webm training_videos/video1.mp4 output/video1.webm
   .venv/bin/python .claude/skills/video-processor/scripts/video_processor.py transcribe training_videos/video1.mp4 output/video1.txt --model base
